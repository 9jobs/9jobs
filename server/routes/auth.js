...
router.post("/contact", contactLimiter, async (req, res) => {
  try {
    ...
    if (!firstName || !phone || !email || !message) {
      if (req.accepts('json')) return res.status(400).json({ error: "All fields are required." });
    ...
    if (!isValidEmail(email)) {
      if (req.accepts('json')) return res.status(400).json({ error: "Please enter a valid email address." });
      return res.status(400).render("partials/contact", {
